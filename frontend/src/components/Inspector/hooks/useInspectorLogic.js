...
export const useInspectorLogic = ({
  ...
  const [cropData, setCropData] = useState({
      width: 0, height: 0, x: 0, y: 0, rotate: 0,
      imageWidth: 0, imageHeight: 0 // Track natural size for clamping
  });
  ...
  const onCropperChange = useCallback((cropper) => {
      ...
      if (coords) {
        // Check for rotation in state if available?
        // react-advanced-cropper stores rotation in transforms.
        // We can get it via `cropper.getTransforms()`? Not always exposed on the event object.
        // But we DO track `flip` and probably `rotation` via our own state/ref interactions?
        // Wait, `react-advanced-cropper` manages state internally. 
        // We can inspect it.
        
        // Let's try to get rotation from our REF if possible or just rely on what we set?
        // Actually, if we use the default uncontrolled mode, we might not know the rotation easily unless exposed.
        // `cropper.getTransforms()` is the method.
        
        /* 
           NOTE: react-advanced-cropper `getCoordinates` returns coordinates 
           relative to the ORIGINAL image, *accounting* for rotation?
           No, usually consistent with original image.
        */
        
        // We need Rotation for "Visual Swapping" logic in stats.
  ...
  const handleSave = useCallback(() => {
    ...
    const rotate = state?.transforms?.rotate || 0;
    const currentFlip = state?.transforms?.flip || { horizontal: false, vertical: false };
    ...
    const autoOutputWidth = outputWidth || (
        (coords.width > (state?.imageSize?.width || 0) && (state?.imageSize?.width || 0) > 0) 
  ...
  const navigateNext = useCallback(() => { if (hasNext) { if (isDirty) handleSave(); onNext(); } }, [hasNext, isDirty, handleSave, onNext]);
  const navigatePrev = useCallback(() => { if (hasPrev) { if (isDirty) handleSave(); onPrev(); } }, [hasPrev, isDirty, handleSave, onPrev]);
  ...
  const handleFlip = (horizontal) => { 
      ...
      setFlip(prev => ({
          ...prev,
          [horizontal ? 'horizontal' : 'vertical']: !prev[horizontal ? 'horizontal' : 'vertical']
      }));
  };
  ...
  const handleSelectionDimChange = (dim, val) => {
      if (dim === 'w') setManualW(val); else setManualH(val);
  ...
  const handleDimBlur = () => { setManualW(''); setManualH(''); };
  
  const handleResizeToggle = () => { setOutputWidth(prev => prev ? null : 1024); setManualOutputWidth(''); setIsDirty(true); };
  const handleOutputWidthChange = (val) => { setManualOutputWidth(val); const num = parseInt(val); if (!isNaN(num)) { setOutputWidth(num); setIsDirty(true); } };
  const handleOutputWidthBlur = () => { setManualOutputWidth(''); };
  ...
  return {
    ...
    manualW, manualH, manualOutputWidth,
