...
function getDefaultTransforms(state, settings) {
    var transforms = tslib.__assign(tslib.__assign({}, state.transforms), { flip: tslib.__assign({}, state.transforms.flip) });
    if (settings.defaultTransforms) {
        var defaultTransforms = index.isFunction(settings.defaultTransforms)
            ? settings.defaultTransforms(state, settings)
            : settings.defaultTransforms;
        if (index.isNumber(defaultTransforms.rotate)) {
            transforms.rotate = defaultTransforms.rotate;
        }
        if (defaultTransforms.flip) {
            if (index.isBoolean(defaultTransforms.flip.horizontal)) {
                transforms.flip.horizontal = defaultTransforms.flip.horizontal;
            }
            if (index.isBoolean(defaultTransforms.flip.vertical)) {
...
exports.getDefaultTransforms = getDefaultTransforms;
